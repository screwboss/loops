/**
@author screwboss
@title  Germs or soot from spirited away
@desc   Altered Balls iteration 
*/
...
export function main(coord, context, cursor, buffer) {
	const t = context.time * 0.00013 + 7
...
st.y *= z
	...
	const s = map(sin(t * 5), -1, 1, 0.0, 0.9)
	...
	for (let i=0; i<num; i++) {
		...
		const x = map(cos(t * 0.53 * (i / num * PI + PI)), -1, 1, -1.2, 1.2)
