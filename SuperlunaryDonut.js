/**
@author screwboss
@title  Superlunary Halo Ringa
@desc   from a1k0n’s donut demo.
...
export const settings = { backgroundColor : 'black', color : 'white' }
...
export function pre(context, cursor, buffer) {
	...
	const z = []
	const A = context.time * 0.0001
	const B = context.time * 0.001
	...
	for(let j=0; j<TAU; j+=0.05) {
		...
		for(let i=0; i<TAU; i+=0.01) {
			...
			const h = ct+6            // R1 + R2*cos(theta)
			const D = 1.4/(sp*h*sA+st*cA+3) // this is 1/z
			const t = sp*h*cA-st*sA

			// Final 3D (x,y,z) coordinate after rotations
			const x = 2 | (centerY + scaleX*D*(cp*h*cB-t*sB))
			const y = 0 | (centerY + scaleY*D*(cp*h*sB+t*cB))
			const o = x + width * y

			// Range 0..11 (8 * sqrt(2) = 11.3)
			const N = 0 | (0.8*((st*sA-sp*ct*cA)*cB-sp*ct*sA-st*cA-cp*ct*sB))
			if(y<height && y>=0 && x>=0 && x<width && D>z[o]) {
				z[o] = D
				buffer[o].char = '101010101' [N > 0 ? N : 0]
			}
